...
if result and Path(result["video_path"]).exists():
    ...
    if sa_info is None:
        st.info(
            "**Save to Google Drive** is not configured yet. Add a service "
            "account under `gcp_service_account` in Streamlit secrets and "
            "share the destination folder with it. See README → "
